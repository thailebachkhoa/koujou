...
bool UnitList::insert(Unit *unit)
{
    ...
    // kiểm tra xem có đơn vị nào cùng loại không
    ...
    // nếu không có đơn vị nào cùng loại, tạo một đơn vị mới
    try
    {
...
bool UnitList::isContain(VehicleType vehicleType)
{
    for (UnitNode *current = head; current != nullptr; current = current->next)
    {
        ...
        else return false; // unitclassofy == INFANTRY
    }
    return false;
}
...
Vehicle *UnitList::getVehicleViaType(VehicleType vehicleType) // return vehicle if it exists
{
    for (UnitNode *current = head; current != nullptr; current = current->next)
    {
        ...
        else break; // unitclassofy == INFANTRY
    }
    return nullptr;
}
...
void Urban::getEffectOnARVN(ARVN *ARVN)
{
    ...
    {
        Unit *unit = ARVN->getUnitList()->getUnit(i);
        if (unit->getUnitClassify() == UnitClassify::VEHICLE) continue;
        Infantry *infantry = static_cast<Infantry *>(unit);
        if (infantry->getInfantryType() != InfantryType::SPECIALFORCES) continue;
